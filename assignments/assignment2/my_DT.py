...
class my_DT:
    ...
    def impurity(self, labels):
        ...
        if self.criterion == "gini":
            ...
            impure = 0
            for k in stats:
                impure -= (stats[k]/ N) * np.log2(stats[k] / N)
    ...
    def find_best_split(self, pop, X, labels):
        ...
        ######################
        best_feature = None
        popsize = float(len(labels[pop]))
        for feature in X.keys():
            cans = np.array(X[feature][pop])
            impurity = self.impurity(labels[pop])
            iln = pop[cans <= cans[0]]
            rln = pop[cans > cans[0]]
            wisl = (float(len(labels[iln])) / popsize) * self.impurity(labels[iln])
            wisr = (float(len(labels[rln])) / popsize) * self.impurity(labels[rln])
            mIpurity = impurity - (wisl + wisr)
            best_feature_to_split = (feature, mIpurity, cans[0], [iln, rln], [self.impurity(labels[iln]), self.impurity(labels[rln])])
            for can in cans:
                t_iln = pop[cans <= can]
                t_rln = pop[cans > can]
                t_wisl = (float(len(labels[t_iln])) / popsize) * self.impurity(labels[t_iln])
                t_wisr = (float(len(labels[t_rln])) / popsize) * self.impurity(labels[t_rln])
                t_mIpurity = impurity - (t_wisl + t_wisr)
                if t_mIpurity > mIpurity:
                    mIpurity = t_mIpurity
                    best_feature_to_split = (feature, mIpurity, can, [t_iln, t_rln], [self.impurity(labels[t_iln]), self.impurity(labels[t_rln])])
            if best_feature != None:
                if best_feature[1] <= best_feature_to_split[1]:
                    best_feature = best_feature_to_split
            else:
                best_feature = best_feature_to_split
